/* ===========================================================================
   POSTCARDS & PLANS — teacher.js  (Trip Log)
   =========================================================================== */
(function () {
  'use strict';
    ...
    api.detail(id).then(function (d) {
      ...
      if (podKeys.length || withMedia.length) {
        ...
        h += '<details class="disc"><summary>Stage resources used<span class="count">' +
          touched + ' of ' + withMedia.length + ' legs opened</span></summary>' +
          '<div class="disc-body"><div class="logscroll"><table class="logtable">' +
          '<thead><tr><th>Leg</th><th>Podcast</th><th>Listened</th><th>Slides</th><th>Video</th><th>Last</th></tr></thead><tbody>';
        if (!withMedia.length) {
        ...
        h += '</tbody></table></div><p class="tiny" style="margin-top:8px">' +
          'Rows in red are legs where this student opened none of the material. Someone stuck on a leg ' +
  ...
  function printPaper(name, level, n) {
    var paper = E.Bank.verifyPaper(level, n);
    var h = '<h2>English Tenses — Level Check</h2>' +
  ...
  $('#t-csv').addEventListener('click', function () {
    var rows = [['student_id', 'name', 'level', 'rank', 'readiness_pct', 'accuracy_pct', 'items_seen', 'items_correct', 'streak', 'longest_streak', 'xp', 'badges', 'last_active', 'lessons_cleared', 'legs_cleared', 'standby_items', 'top_weakness']];
    T.roster.forEach(function (s) {
      var p = s.progress || P.blank(s.id, s.name);
      var lvl = P.stageClearedCount(p);
    ...
    a.download = 'postcards-and-plans-' + E.today() + '.csv';
